...
async function main(): Promise<void> {
  ...
  try {
    ...
    await page.route('**/api/vo-items**', async (route) => json(route, { data: [] }));
    ...
    expect(metrics.trianglePath).toBe('M21 13H43L32 28Z');
    expect(metrics.stemPath).toBe('M32 28V51');
    expect(metrics.baselinePath).toBe('M11 43H53');
    await expect(brandMark).toBeVisible();

    await page.screenshot({ path: path.join(EVIDENCE_DIR, 'n05r2-01-header-closed-390x844.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(EVIDENCE_DIR, 'n05r2-02-profile-open-390x844.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(EVIDENCE_DIR, 'n05r2-03-header-in-entry-flow-390x844.png'), fullPage: false });

    console.log(`N05R.2 gate captured ${MOBILE.width}x${MOBILE.height}: accepted app-icon mark + FPTV UPSI + R03 + profile`);
  } finally {
    await context.close();
    await browser.close();
  }
