...
async function main(): Promise<void> {
  ...
  const specimenCss = `
    :root {
      ...
      --font-geist-mono: "Courier New", ui-monospace, monospace;
    }
    * { box-sizing: border-box; }
    ...
    .hero { margin-top: 12px; padding: 18px 18px 16px; border-top: 1px solid var(--ng-rule); border-bottom: 1px solid var(--ng-rule); background: var(--ng-graphite-925); }
    .hero-mark { display: grid; place-items: center; margin-bottom: 12px; }
    .hero-mark > svg { width: 118px; height: 94px; color: var(--ng-ivory-100); }
    ...
    .sample { min-height: 90px; padding: 11px; border-top: 1px solid var(--ng-rule); border-bottom: 1px solid var(--ng-rule); background: var(--ng-graphite-900); }
    .sample-title { margin-bottom: 10px; color: var(--ng-grey-500); font: 600 8px/1 var(--ng-font-reference); letter-spacing: .11em; text-transform: uppercase; }
    .sample-mark { width: 48px; height: 38px; color: var(--ng-ivory-100); }
    .sample-mark svg { width: 100%; height: 100%; }
    .type-sample { margin-top: 15px; padding-top: 13px; border-top: 1px solid var(--ng-rule); }
    .work { margin-top: 8px; color: var(--ng-ivory-100); font: 480 14px/1.3 var(--ng-font-work); }
    .ref { margin-top: 7px; color: var(--ng-grey-300); font: 500 10px/1.25 var(--ng-font-reference); letter-spacing: .045em; }
    .brand { margin-top: 7px; color: var(--ng-ivory-100); font: 620 12px/1 var(--ng-font-brand); letter-spacing: .14em; text-transform: uppercase; }
    .tokens { display: grid; grid-template-columns: repeat(4,1fr); gap: 7px; margin-top: 15px; }
    .token { height: 32px; border: 1px solid var(--ng-graphite-650); position: relative; }
    ...
    .baseline-note { margin-top: 12px; color: var(--ng-grey-500); font: 500 9px/1.35 var(--ng-font-reference); letter-spacing: .035em; }
  `;
  ...
  await page.screenshot({ path: path.join(EVIDENCE_DIR, 'n01-brand-gate-375x812.png') });
  console.log(`N01 visual gate captured: ${metrics.viewportWidth}x${metrics.viewportHeight}px wordmark=${metrics.wordmark.toFixed(1)}px mark=${metrics.mark.toFixed(1)}px`);
