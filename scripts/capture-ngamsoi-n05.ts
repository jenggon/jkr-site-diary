...
async function main(): Promise<void> {
  ...
  expect(metrics.source.after).toBe('');
  expect(metrics.dates.after).toBe('');
  expect(metrics.site.after).toBe('');
  expect(metrics.workforce.after).toBe('');
  expect(metrics.warning.after).toBe('');

  // Mobile intentionally hides state words; verify state colors remain distinct at the node level.
