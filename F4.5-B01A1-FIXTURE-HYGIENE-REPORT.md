...
1. All synthetic IDs were migrated to RFC 4122 version 4 compliant UUIDs.
...
## 4. Confirmation of Product Code Invariance
...
## 6. Live Runtime Proof & Discovery

### A. UUID Validation Error Resolution
Upon logging in as `submitter@jkr.gov.my` and selecting **C01 Test Programme**, the previous error banner:
```
Missing or invalid query parameter: programmeId
```
...
### B. Downstream Product Code Discovery (Boundary Stop)
With valid UUID parameter passing restored, the request `GET /api/programme-revision?programmeId=11111111-1111-4111-8111-111111111111` progresses to `SiteDiaryManagementReadRepository.ts` where it triggers HTTP 500 (`Internal server error`). 
...
`records_mobile_375x812.png`:

![Mobile Records Viewport (375x812)](file:///C:/Users/user/.gemini/antigravity/brain/5494aaf0-afa4-4675-a7eb-d453d3ebff65/records_mobile_375x812.png)

### Desktop Records (1280 × 720)
`records_desktop_1280x720.png`:

![Desktop Records Viewport (1280x720)](file:///C:/Users/user/.gemini/antigravity/brain/5494aaf0-afa4-4675-a7eb-d453d3ebff65/records_desktop_1280x720.png)
...
1. Pull Request [#71](https://github.com/jenggon/jkr-site-diary/pull/71) is open against `develop` and ready for review.
2. The synthetic fixture UUID standard has been permanently enforced across the repository.
3. Recommend approving PR #71 and authorizing a targeted product-code follow-up task to repair the query syntax in `SiteDiaryManagementReadRepository.ts` (`revision_name` column name and explicit `programme!programme_revision_programme_id_fkey` relation qualifier).
