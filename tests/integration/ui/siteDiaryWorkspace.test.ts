...
vi.mock('@/app/site-diary/DailyEntryForm', () => ({
  default: (props: { initialTab: string; hideModeNavigation: boolean }) => {
    ...
    return React.createElement('div', null,
      ...
      React.createElement('button', { onClick: () => setSelected(true) }, 'SELECT_ACTIVITY_OR_EDIT'),
      selected && React.createElement('div', null, 'ACTIVITY_A_EDIT_TARGET'),
    );
  },
}));
vi.mock('@/app/site-diary/ApprovalQueue', () => ({
  default: (props: { onSelectReview: (siteDiaryId: string, approvalId: string) => void }) => React.createElement(
    'div', null,
...
vi.mock('@/app/site-diary/ApprovalReview', () => ({
  default: (props: { siteDiaryId: string; approvalId: string }) => React.createElement(
    'div', null, `DECISION_CONTROLS_${props.approvalId}_${props.siteDiaryId}`
  ),
}));
...
describe('Site Diary runtime workspace navigation', () => {
  ...
  it('preserves the tab category but remounts Programme-owned record and entry state', async () => {
    await act(async () => root.render(React.createElement(SiteDiaryWorkspace)));
    const selectHistory = [...container.querySelectorAll('button')]
      .find((button) => button.textContent === 'SELECT_HISTORICAL_DIARY') as HTMLButtonElement;
    ...
    await act(async () => (tabs[1] as HTMLButtonElement).click());
    const selectActivity = [...container.querySelectorAll('button')]
      .find((button) => button.textContent === 'SELECT_ACTIVITY_OR_EDIT') as HTMLButtonElement;
  ...
  it('closes an exact Approval review immediately when Programme ownership changes', async () => {
    ...
    const openReview = [...container.querySelectorAll('button')]
      .find((button) => button.textContent === 'OPEN_REVIEW') as HTMLButtonElement;
  ...
  it('keeps all contextual destinations reachable and active state identifiable on mobile and desktop', async () => {
    ...
    for (const [index, tab] of tabs.entries()) {
      ...
      expect(currentTabs.filter((item) => item.getAttribute('aria-selected') === 'true')).toHaveLength(1);
      expect(currentTabs[index]?.className).toContain('bg-blue-600');
    }
