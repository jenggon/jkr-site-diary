...
describe('F2.7-C06 Programme patch atomic authority contract', () => {
  ...
  beforeAll(() => {
    ...
    migrationSql2 = readFileSync(
      join(
        __dirname,
        '../../supabase/migrations/20260824143901_f2_7_c06_programme_update_capability_and_fixes.sql',
      ),
      'utf8',
    );
  ...
  it('preserves public.programme protection by using a SECURITY DEFINER wrapper', () => {
    const wrapper = migrationSql2.match(/CREATE OR REPLACE FUNCTION "private"\."c06_update_programme_core"[\s\S]*?\$\$;/)?.[0];
  ...
  it('enforces actor integrity with a27_assert_actor', () => {
    expect(migrationSql2).toMatch(/PERFORM "private"\."a27_assert_actor"\(p_actor_id\);/);
  });

  it('enforces programme authority using assert_capability and PROGRAMME_UPDATE', () => {
    expect(migrationSql2).toMatch(/PERFORM "private"\."assert_capability"\(p_actor_id, p_programme_id, 'PROGRAMME_UPDATE'\);/);
    expect(migrationSql2).toMatch(/INSERT INTO "public"\."permission"[\s\S]*'PROGRAMME_UPDATE'/);
  });
