...
describe('ProgrammeRowMapper', () => {
  ...
  it('should map ProgrammeRevisionRow to ProgrammeRevision domain entity', () => {
    const revisionRow: ProgrammeRevisionRow = {
      ...
      programme_id: 'prog-1',
      revision_number: 1,
      revision_title: 'Baseline Revision',
      is_current: true,
      status: 'Approved',
      description: 'Initial',
    ...
    const revisionDomain = mapper.toRevisionDomain(revisionRow);
  ...
  it('should map ProgrammeRevision domain entity back to ProgrammeRevisionRow', () => {
    ...
    expect(revisionRow.is_current).toBe(true);
    expect(revisionRow.description).toBeNull();
  });
});
