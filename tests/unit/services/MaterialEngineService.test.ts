...
describe('MaterialEngineService', () => {
  ...
  it('resolves via Priority 1: MSP Material', async () => {
    const mspRepo: IMspMaterialRepository = { findMaterialsByMspTask: async () => [mockItem] };
  ...
  it('filters substitutions and optional materials based on policy', async () => {
    const mspRepo: IMspMaterialRepository = { findMaterialsByMspTask: async () => [
      mockItem, 
      { ...mockItem, isMandatory: false }
    ] };
    const tradeRepo: ITradeMaterialLibraryRepository = { getMaterialCompositionByTrade: async () => null };
    const registry = new MaterialRuleEvaluatorRegistry();
  ...
  it('resolves via Priority 2: Trade Library', async () => {
    const mspRepo: IMspMaterialRepository = { findMaterialsByMspTask: async () => null };
