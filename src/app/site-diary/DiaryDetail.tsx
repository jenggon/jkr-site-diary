...
export default function DiaryDetail({ projection, programmeId, onBack, onEdit }: DiaryDetailProps) {
  ...
  if (error && !detail) return <div role="alert" data-record-detail-state="error" className="rounded-none border border-red-800 bg-red-950/40 p-4"><p>{error}</p><div className="mt-3 flex gap-3"><button type="button" onClick={load} className="min-h-[44px] font-bold underline">Cuba Semula</button><button type="button" onClick={onBack} data-record-back className="min-h-[44px] font-bold">← Kembali ke Senarai Rekod</button></div></div>;
  ...
  return <article aria-label="Butiran Buku Harian Tapak" className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
    <div className="flex items-center justify-between gap-3"><button type="button" onClick={onBack} data-record-back className="min-h-[44px] font-bold">← Kembali ke Senarai Rekod</button>{historical && <span className="text-xs font-bold text-amber-300">Sejarah / Baca Sahaja</span>}</div>
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      {printableSiteDiaryId && <Link href={`/site-diary/print?id=${encodeURIComponent(printableSiteDiaryId)}`} data-record-print-exact className="flex min-h-[44px] items-center justify-center rounded-none border border-zinc-700 bg-zinc-800 px-4 font-bold text-white">Cetak Rekod Ini</Link>}
