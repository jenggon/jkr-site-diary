...
export default function DiaryDetail({ projection, programmeId, onBack, onEdit }: DiaryDetailProps) {
  ...
  return <article aria-label="Butiran Buku Harian Tapak" className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
    ...
    <header><p className="text-xs text-blue-300">{projection.sourceType ?? FALLBACK} · {display(projection.sourceReference)}</p><h2 className="mt-1 text-xl font-bold">{display(projection.activityTitle)}</h2><p className="mt-1 text-sm text-zinc-400">Semakan {revision.revisionNumber} — {revision.revisionTitle} · {revision.revisionStatus}</p></header>
    {error && <div role="alert" className="rounded-xl border border-amber-700 bg-amber-950/30 p-3 text-sm text-amber-200">{error}</div>}
