...
BEGIN
    ...
    VALUES 
        (v_p1_user_id, 'authenticated', 'authenticated', 'submitter@jkr.gov.my', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now(), '', '', '', ''),
        (v_p2_user_id, 'authenticated', 'authenticated', 'reviewer@jkr.gov.my', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now(), '', '', '', ''),
        (v_p3_user_id, 'authenticated', 'authenticated', 'unauthorized@jkr.gov.my', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now(), '', '', '', '')
    ...
    INSERT INTO public.programme_revision (revision_id, programme_id, revision_no, revision_name, status, created_by) VALUES
        (v_rev_hist, v_prog_id, 1, 'Historical Revision', 'Archived', v_p1_user_id),
    ...
    INSERT INTO public.site_diary (site_diary_id, programme_id, activity_id, revision_id, activity_date, notes, status, submitted_by) VALUES
        ('55555555-5555-5555-5555-555555555553', v_prog_id, v_act_msp, v_rev_cur, '2026-08-11', 'Pending Approval', 'In Progress', v_p1_user_id)
